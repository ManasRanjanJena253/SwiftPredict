...
export default function SwiftPredictUI() {
  ...
  // Clear error/success messages when switching tabs or performing new actions
  const clearMessages = () => {
    setError('');
    setSuccess('');
  };
  ...
  const fetchProjects = async () => {
    ...
    try {
      ...
      if (!mlResponse.ok || !dlResponse.ok) {
      ...
      // Handle API response structure - API returns arrays on success or objects with Error key
      setProjects({
        ml: Array.isArray(mlData) ? mlData : [],
        dl: Array.isArray(dlData) ? dlData : []
      });

      // Show appropriate messages if no projects found
      if (!Array.isArray(mlData) && mlData.Error) {
        console.log('ML Projects:', mlData.Error);
      }
      if (!Array.isArray(dlData) && dlData.Error) {
        console.log('DL Projects:', dlData.Error);
      }
  ...
  const fetchProjectsByStatus = async (status) => {
    ...
    try {
      ...
      setError('Failed to fetch projects by status: ' + err.message);
      setStatusProjects([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleLogSubmit = async (e) => {
    ...
    try {
      ...
      switch (logForm.action) {
        case 'log_param':
          // Query parameters for log_param
          url = `${API_BASE}/${logForm.projectName}/runs/${logForm.runId}/log_param?key=${encodeURIComponent(logForm.key)}&value=${encodeURIComponent(logForm.value)}`;
          break;
        case 'add_tags':
          // JSON body for add_tags
          url = `${API_BASE}/${logForm.projectName}/runs/${logForm.runId}/add_tags`;
          requestOptions.body = JSON.stringify(logForm.tags.split(',').map(tag => tag.trim()));
          break;
        case 'update_status':
          // Query parameters for update_status
          url = `${API_BASE}/${logForm.projectName}/runs/${logForm.runId}/update_status?status=${encodeURIComponent(logForm.status)}`;
          break;
        case 'add_notes':
          // Query parameters for add_notes
          url = `${API_BASE}/${logForm.projectName}/runs/${logForm.runId}/add_notes?notes=${encodeURIComponent(logForm.notes)}`;
          break;
        default:
      ...
      setError('Failed to submit log data: ' + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteProject = async (projectName, runId = '') => {
    ...
    try {
      const url = runId
        ? `${API_BASE}/projects/delete?project_name=${encodeURIComponent(projectName)}&run_id=${encodeURIComponent(runId)}`
        : `${API_BASE}/projects/delete?project_name=${encodeURIComponent(projectName)}`;
            ...
            {availableMetrics.length > 0 && (
              <div className="bg-white rounded-lg shadow-sm border p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {availableMetrics.map((metricData, idx) => (
                    <div key={idx} className="bg-gray-50 p-4 rounded-lg">
                      <p className="font-medium">Run ID: {metricData.run_id}</p>
                      {metricData.metrics && metricData.metrics.metric && (
                        <div className="text-sm text-gray-600 mt-2">
                          <p>Metrics:</p>
                          <ul className="list-disc list-inside">
                            {Array.isArray(metricData.metrics.metric) ?
                              metricData.metrics.metric.map((metric, i) => (
                                <li key={i}>{metric}</li>
                              )) :
                              <li>{metricData.metrics.metric}</li>
                            }
                          </ul>
                        </div>
                      )}
